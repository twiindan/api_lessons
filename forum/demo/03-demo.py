...
for message_id in range(0, 4):
    ...
    response = requests.post(url=FORUM_URL, data=ujson.dumps(message_body))
    assert_true(response.ok)
    assert_equals(response.text, 'message created')


#WE CAN MAKE A REQUEST TO OBTAIN ALL THE FORUM MESSAGES
response = requests.get(url=FORUM_URL)
assert_true(response.ok)
...
response = requests.get(url=FORUM_URL, params=payload)
print (response.url)
